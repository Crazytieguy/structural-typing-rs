...
//!
//! This example uses a functional style with standalone validation functions.
...
// Standalone validation functions (pure, no side effects)
fn validate_email(email: &str, used: &HashSet<String>) -> Result<String, ValidationError> {
...
fn validate_username(username: &str, used: &HashSet<String>) -> Result<String, ValidationError> {
    if username.len() < 3 {
...
fn validate_terms(accepted: bool) -> Result<bool, ValidationError> {
    if !accepted {
...
// Registry for tracking used usernames/emails
struct UserRegistry {
...
impl UserRegistry {
    ...
    fn register(&mut self, validated: &FullyValidated) {
...
fn main() {
    ...
    match stage1_basic_info(&raw, &registry)
        .and_then(|v1| stage2_security(&raw, v1))
        .and_then(|v2| stage3_finalize(&raw, v2))
    {
        Ok(validated) => {
    ...
    let after_stage1 = stage1_basic_info(&raw, &registry).unwrap();
    println!("   ✓ Stage 1 (Basic Info): Email and username validated");

    let after_stage2 = stage2_security(&raw, after_stage1).unwrap();
    println!("   ✓ Stage 2 (Security): Password validated");
    println!("   Can login: {}", after_stage2.can_login());

    let after_stage3 = stage3_finalize(&raw, after_stage2).unwrap();
    println!("   ✓ Stage 3 (Finalize): Age and terms validated");

    registry.register(&after_stage3);
    ...
    match stage1_basic_info(&merged, &registry)
        .and_then(|v1| stage2_security(&merged, v1))
        .and_then(|v2| stage3_finalize(&merged, v2))
    {
        Ok(validated) => {
    ...
    println!("✓ Functional style with pure validation functions");
}
