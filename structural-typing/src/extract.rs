...
pub trait Extract<A: Access<T>, T>: Access<T> + Sized {
...
impl<T, A: Access<T>> TryExtract<PhantomData<T>, T> for A {
    fn try_extract(
        self,
    ) -> Result<
        (
    ...
    >
    where
        PhantomData<T>: Access<T>,
    {
        Ok(self.extract())
    }
}

impl<T, A: Access<T>> TryExtract<Option<T>, T> for A {
    fn try_extract(self) -> Result<(Option<T>, <Option<T> as Access<T>>::RemainderFrom<Self>), Self>
    where
        Option<T>: Access<T>,
    {
        Ok(self.extract())
...
impl<T> TryExtract<T, T> for T {
    fn try_extract(self) -> Result<(T, <T as Access<T>>::RemainderFrom<Self>), Self> {
        Ok(self.extract())
    }
}

impl<T> TryExtract<T, T> for Option<T> {
    fn try_extract(self) -> Result<(T, <T as Access<T>>::RemainderFrom<Self>), Self> {
        match self {
